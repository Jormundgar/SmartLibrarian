...
async function getLines() {
    lines = await $.ajax({
        url: "http://localhost:8070/api/readers/number",
        method: "GET",
        dataType: 'json'
    });
}
...
async function getId(id, type) {
    let json = await $.ajax({
        url: `http://localhost:8070/api/readers/item?id=${id}`,
        method: 'GET',
        dataType: 'json'
    });
    if (type === 'delete') {
...
async function deleteReader() {
    let id = $("#delete-id").val();
        ...
        await $.ajax({
            url: `http://localhost:8070/api/readers?id=${id}`,
            method: 'DELETE',
            success: function () {
                getReaders();
            }
...
async function getReaders() {
        ...
        let readerJson = await $.ajax({
            url: `http://localhost:8070/api/readers?byYear=${byYear}&pageNumber=${page}`,
            method: "GET",
            dataType: 'json'
        });
        readersTBody.empty();
...
async function getBooks(id) {
    let books = await $.ajax({
        url: `http://localhost:8070/api/books/reader?id=${id}`,
        method: 'GET',
        dataType: 'json'
    });
